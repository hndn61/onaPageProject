...
//Artırma fonksiyonu: çağrıldığında arttırma işlemini yapıp tekrar againCall() fonksiyonunu çağırır.
...
let counterPlus2 = function () {
    counter2++;
    againCall2();
}

let counterMinus2 = function () {
    counter2--;
    againCall2();
}

let counterPlus3 = function () {
    counter3++;
    againCall3();
}

let counterMinus3 = function () {
    counter3--;
    againCall3();
}
            ...
            <div className="container">
                <div className="row">
                    <div className="col-md-8">
                        {/* Sayaç değerini almak için sayaç değişkenini çağırır.*/}
                        <p id="p1">1.Forma Adeti: {counter}</p>
                    </div>
                    <div className="col-md-2">
                        ...
                        <button
                            ...
                            //CSS yapıları
                            style={{ fontSize: '14px', backgroundColor: "brown", borderColor: "brown", borderRadius: "0.25rem", boxSizing: "border-box", width: "30px"}}
                            id="plus_id"
                        >
                            +
                        </button>
                    ...
                    <div className="col-md-2">
                        ...
                        <button
                            ...
                            className="btn"
                            //CSS yapıları
                            style={{ fontSize: '14px', backgroundColor: "blue", borderColor: "blue",  borderRadius: "0.25rem", boxSizing: "border-box", width: "30px"}}
                            id="minus_id"
                        >
                            -
                        </button>
    ...
    var eventTuto2 = (
        <div>
            <div className="container">
                <div className="row">
                    <div className="col-md-8">
                        <p id="p2">2.Forma Adeti: {counter2}</p>
                    </div>
                    <div className="col-md-2">
                        {' '}
                        <button
    ...
    var eventTuto3 = (
        <div>
            <div className="container">
                <div className="row">
                    <div className="col-md-8">
                        <p id="p3">3.Forma Adeti: {counter3}</p>
                    </div>
                    <div className="col-md-2">
                        {' '}
                        <button
...
function timerClock() {
    var time = (
        <div>
            <p> 
                {/* Kullanılan işletim sisteminin saat düzenini kullanarak, bir date nesnesinin sadece saat kısmını yerel düzene uygun string tipine dönüştürerek verir. */}
                <b> {new Date().toLocaleTimeString()} </b>
            </p>
        </div>
    )
    //Ekranda template(time)-root(timer_id) yapısını gösterir.
...
$(function(){
    ...
    $("#gosterButon").click(function (){
        ...
        $("#timer_id").show();
            //Bir metodu sürekli belirli aralıklarla çağırır.
    ...
    $("#kapatButon").click(function (){
        ...
        $("#timer_id").hide();
            //setInterval: timerClock() fonksiyonunu sürekli belirli aralıklarla çağırır.
            setInterval(timerClock, 1000);
    });
});
/* Timer ends codes */
...
function formOnSubmit(event) {
    ...
    //formInput'ın value değerini event yapısı ile alır.
    ...
    if (formInputData) {
        ...
        //Form bileşenini görmek için çağırdık.
        formTutorial();
        //Son durumda input içini boşaltır.
...
function formTutorial() {
    ...
    var form = (
        <div>
            {/* Title bileşenin içeriğini gösterir. */}
            <p className="text-capitalize mt-2" style={{color:"brown",fontSize:"15px"}}>{listItemObject.title} <i
            ...
            <p className="text-capitalize mt-2" style={{color:"black",fontSize:"14px"}}>Ekleme listesi</p>
            {
                <ul style={{color:"black",fontSize:"13px"}}>
                    {/* Data dizisindeki elemanları alır. */}
                    {/* map: elemanları teker teker işleme alır. */}
                    {listItemObject.data.map((temp) => {
                        //toString: kendisine tanımlanan değişken değerini string olarak değiştirerek döndürür.
                        return <li className="text-capitalize" key={temp.toString()} > {temp} </li>
                    })} 
                </ul>
            }
            {/* onSubmit */}
            {/* onSubmit: Form bileşenin submit olmamasıdır.(Submit yapmadan önce bir yere göndermesi için kullanılır.) */}
            <form onSubmit={formOnSubmit}>
                <div className="row">
                    <div className="col-md-6">
                        <input type="text" className="form-control" name="formInput" style={{color:"black",fontSize:"13px", marginTop:"3px"}}/>
                    </div>

                    <div className="col-md-6">
                        {/* Gönder butonuna tıkladıkça yazdığımız elemanları listeleyerek gösterir. */}
                        <button className="btn btn-danger mr-3" style={{fontSize:"0.75rem",padding:"0 1rem",marginBottom: "1rem",marginRight: "1rem"}} type="submit">
                            Gönder
                        </button>
                        {/* Temizle butonuna tıkladıkça yazdığımız elemanları siler. */}
                        <button onClick={clearList} className="btn btn-primary" style={{fontSize:"0.75rem",padding:"0 1rem",marginBottom: "1rem",marginRight: "1rem"}}>
                            Temizle
                        </button>
                    </div>
                </div>
            ...
            {/* Arama butonu */}
            <form className="d-flex mb-30">
                <input className="form-control" style={{marginRight:"25px", marginTop:"20px", fontSize:"13px", color:"black"}} type="text" placeholder="arama ..." />
                <button className="btn" style={{backgroundColor: "brown", fontSize:"0.75rem",padding:"0 1.4rem", marginTop:"18px", marginBottom: "1rem",marginRight: "1rem"}} type="submit">Arama</button>
            </form>
        </div>
    )
    //Ekranda template(form)-root(todolistId) yapısını gösterir.
